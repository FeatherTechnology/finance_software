...
$cp_id = $_POST['cp_id'];
$qry = $pdo->query("SELECT ci.id, ci.cus_profile_id, ci.holder_type, ci.holder_name, ci.relationship, ci.bank_name, cnl.cheque_no 
FROM cheque_info ci 
...
if ($qry->rowCount() > 0) {
    while ($result = $qry->fetch()) {
        ...
        $result['h_person'] = '';
        $result['relation'] = '';
        $result['action'] = "<input type='checkbox' class='noc_cheque_chkbx' name='noc_cheque_chkbx' value='" . $result['id'] . "' >";
