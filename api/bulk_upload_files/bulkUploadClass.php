...
class bulkUploadClass
{
    ...
    public function fetchAllRowData($Row)
    {
        $dataArray = array(
            ...
            'guarantor_live' => isset($Row[19]) ? $Row[19] : "",
            'guarantor_income' => isset($Row[20]) ? $Row[20] : "",
            'residential_type' => isset($Row[21]) ? $Row[21] : "",
            'resident_detail' => isset($Row[22]) ? $Row[22] : "",
            'res_address' => isset($Row[23]) ? $Row[23] : "",
            'native_address' => isset($Row[24]) ? $Row[24] : "",
            'occupation' => isset($Row[25]) ? $Row[25] : "",
            'occ_detail' => isset($Row[26]) ? $Row[26] : "",
            'occ_income' => isset($Row[27]) ? $Row[27] : "",
            'occ_address' => isset($Row[28]) ? $Row[28] : "",
            'area_confirm' => isset($Row[29]) ? $Row[29] : "",
            'area' => isset($Row[30]) ? $Row[30] : "",
            'line' => isset($Row[31]) ? $Row[31] : "",
            'cus_limit' => isset($Row[32]) ? $Row[32] : "",
            'about_cus' => isset($Row[33]) ? $Row[33] : "",
            'loan_category' => isset($Row[34]) ? $Row[34] : "",
            'loan_amount' => isset($Row[35]) ? $Row[35] : "",
            'profit_type' => isset($Row[36]) ? $Row[36] : "",
            'due_method' => isset($Row[37]) ? $Row[37] : "",
            'due_type' => isset($Row[38]) ? $Row[38] : "",
            'profit_method' => isset($Row[39]) ? $Row[39] : "",
            'scheme_due_method' => isset($Row[40]) ? $Row[40] : "",
            'scheme_day' => isset($Row[41]) ? $Row[41] : "",
            'scheme_name' => isset($Row[42]) ? $Row[42] : "",
            'interest_rate' => isset($Row[43]) ? $Row[43] : "",
            'due_period' => isset($Row[44]) ? $Row[44] : "",
            'doc_charge' => isset($Row[45]) ? $Row[45] : "",
            'processing_fees' => isset($Row[46]) ? $Row[46] : "",
            'principal_amnt' => isset($Row[47]) ? $Row[47] : "",
            'interest_amnt' => isset($Row[48]) ? $Row[48] : "",
            'total_amnt' => isset($Row[49]) ? $Row[49] : "",
            'due_amnt' => isset($Row[50]) ? $Row[50] : "",
            'doc_charge_calculate' => isset($Row[51]) ? $Row[51] : "",
            'processing_fees_calculate' => isset($Row[52]) ? $Row[52] : "",
            'net_cash' => isset($Row[53]) ? $Row[53] : "",
            'loan_date' => isset($Row[54]) ? $Row[54] : "",
            'dueStart_date' => isset($Row[55]) ? $Row[55] : "",
            'maturity_date' => isset($Row[56]) ? $Row[56] : "",
            'referred' => isset($Row[57]) ? $Row[57] : "",
            'agent_id' => isset($Row[58]) ? $Row[58] : "",
            'agent_name' => isset($Row[59]) ? $Row[59] : "",
            'payment_mode' => isset($Row[60]) ? $Row[60] : "",
            'issue_amount' => isset($Row[61]) ? $Row[61] : "",
            'transaction_id' => isset($Row[62]) ? $Row[62] : "",
            'cheque_no' => isset($Row[63]) ? $Row[63] : "",
            'issue_date' => isset($Row[64]) ? $Row[64] : "",
            'issue_person' => isset($Row[65]) ? $Row[65] : "",
    ...
    function checkAgent($pdo, $agent_name)
    {
        if ($agent_name != '') { // because it's not mandatory
            $stmt = $pdo->query("SELECT agent_code FROM `agent_creation` WHERE agent_name = '$agent_name'");
            if ($stmt->rowCount() > 0) {
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $agentCheck = $row["agent_code"];
            } else {
    ...
    ");
        ...
        $insert_cp_query = "INSERT INTO customer_profile (
            ...
            '" . strip_tags($data['mobile']) . "', '', '" . strip_tags($data['guarantor_name']) . "', '', '" . strip_tags($data['cus_data']) . "', 
            ...
            '" . strip_tags($data['occ_address']) . "', '" . strip_tags($data['area_confirm']) . "', '" . strip_tags($data['area_id']) . "', '" . strip_tags($data['area_line']) . "', 
        ...
        // Prepare values for loan_entry_loan_calculation table
        ...
        '" . strip_tags($cus_profile_id) . "', '" . strip_tags($data['cus_id']) . "','" . strip_tags($data['loan_id']) .  "', '" . strip_tags($data['loan_category']) . "','" . strip_tags($data['loan_amount']) . "', '" . strip_tags($data['profit_type']) . "','" . strip_tags($data['due_method']) . "','" . strip_tags($data['due_type']) . "',
    ...
    function loanIssueTables($pdo, $data)
    {  
        $user_id = $_SESSION['user_id'];
        // Prepare values for loan_issue table
        $cus_profile_id = isset($data['cus_profile_id']) ? $data['cus_profile_id'] : '';
